...
class TestCompanyAPIViewSet(TestCase):
    ...
    def setUpTestData(cls):
        Company.objects.create(name='company_name', location='company_location', description='company_description')
    ...
    def test_if_company_created_returns_201_created(self):
        data = {"name":"'company_name'", "location": "company_location", "description":"company_description"}
        response = self.client.post('/api/v1/companies/', data)
        self.assertEqual(response.status_code, 201)

    def test_if_survey_is_created_when_company_created_returns_True_if_exists(self):
        data = {"name": "'company_name'", "location": "company_location", "description":"company_description"}
        response = self.client.post('/api/v1/companies/', data)
